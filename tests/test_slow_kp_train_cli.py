...
BASELINE_PATH = Path(__file__).with_name("slow_kp_train_cli_baselines.json")
TEST_SEED = 12345
EVAL_SEED = 12346
...
def _run_case(case: Case, workdir: Path) -> None:
    ...
    subprocess.run(
        [
            sys.executable,
            str(SCRIPT_ROOT / "kp_train_cli.py"),
            "--case",
            str(case.idx),
            "--workdir",
            str(workdir),
...
def test_kp_train_cli(case: Case, tmp_path: Path, request, baseline_store):
