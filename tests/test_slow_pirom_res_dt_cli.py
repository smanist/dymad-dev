...
SCRIPT_ROOT = REPO_ROOT / "scripts" / "pirom_res_dt"
BASELINE_PATH = Path(__file__).with_name("slow_pirom_res_dt_cli_baselines.json")
TEST_SEED = 12345
...
def _eval_rmse(case: Case, checkpoint_path: Path) -> float:
    ...
    with torch.no_grad():
        pred = np.stack(
            [predict_fn(xs[j], ts[0], u=us[j], p=ps[j]) for j in range(len(xs))],
            axis=0,
        )
...
def test_pirom_res_dt_cli(case: Case, tmp_path: Path, request, baseline_store):
