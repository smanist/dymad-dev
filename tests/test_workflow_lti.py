...
ls_opt = {"method": "truncated", "params": 2, "interval": 3, "times": 2, "start_with_ls": False}
trn_wf = {
    "n_epochs": 10,
    "save_interval": 5,
...
cfgs = [
    ...
    (
        ...
        KBF,
        WeakFormTrainer,
        {"model": mdl_kb, "training": trn_wf, "transform_x": trx, "transform_u": tru},
    ),
    (
        "sdm_smp",
        DSDM,
        NODETrainer,
        {"model": mdl_s1, "training": trn_dt, "transform_x": trx, "transform_u": tru},
    ),
    (
        "sdm_std",
        DSDM,
        NODETrainer,
        {"model": mdl_s2, "training": trn_dt, "transform_x": trx, "transform_u": tru},
    ),
...
def test_checkpoint_load_uses_typed_build_model_for_regular_models(
    lti_data, lti_gau, env_setup, monkeypatch
):
    ...
    try:
        predict_case(7, lti_gau, env_setup)
    finally:
        if os.path.exists(env_setup / "lti_model"):
            shutil.rmtree(env_setup / "lti_model")
