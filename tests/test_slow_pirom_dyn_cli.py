...
import pytest
import torch
from scripts.pirom_dyn.dyn_train import DPJ, DPT, f, g, t_grid
...
SCRIPT_ROOT = REPO_ROOT / "scripts" / "pirom_dyn"
BASELINE_PATH = Path(__file__).with_name("slow_pirom_dyn_cli_baselines.json")
TEST_SEED = 12345
...
def _run_case(case: Case, workdir: Path) -> None:
    subprocess.run(
        [
            sys.executable,
            str(SCRIPT_ROOT / "dyn_train_cli.py"),
            "--case",
            str(case.idx),
            "--workdir",
            str(workdir),
...
def _eval_rmse(case: Case, checkpoint_path: Path) -> float:
    sampler = TrajectorySampler(f, g, config=SCRIPT_ROOT / "dyn_test.yaml", rng=case.seed)
    ts, xs, us, ys, ps = sampler.sample(t_grid, batch=5)
    _, predict_fn = load_model(case.model_class, checkpoint_path)
    with torch.no_grad():
        pred = np.stack(
            [predict_fn(ys[j], ts[0], u=us[j], p=ps[j]) for j in range(len(ys))],
            axis=0,
        )
...
def test_pirom_dyn_cli(case: Case, tmp_path: Path, request, baseline_store):
