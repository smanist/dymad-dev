...
def train_single_method(trainer_class, method_name, method_key):
    ...
    try:
        ...
        logger.info(
            f"{method_name} - Model parameters: {sum(p.numel() for p in trainer.model.parameters())}"
        )
        ...
        logger.info(
            f"{method_name} - Starting training for {n_epochs} epochs with plots every {save_interval} epochs"
        )
...
def create_final_comparison_plots(node_results, weak_results):
    ...
    # RMSE History
    axes[0, 2].semilogy(
        node_results["rmse_epochs"],
        node_results["rmse_history"],
        "b-",
        label="NODE",
        linewidth=2,
        marker="o",
    ...
    axes[0, 2].semilogy(
        weak_results["rmse_epochs"],
        weak_results["rmse_history"],
        "r-",
        label="Weak Form",
        linewidth=2,
        marker="s",
    ...
    logger.info("Final prediction MSE comparison:")
