...
def main():
    ...
    for epoch in range(start_epoch, config['training']['n_epochs']):
        ...
        if epoch % config['training']['save_interval'] == 0:
            ...
            rmse_train = prediction_rmse(model, traj_train, ts, metadata, plot=False)
            rmse_val = prediction_rmse(model, traj_val, ts, metadata, plot=False)
            rmse_test = prediction_rmse(model, traj_test, ts, metadata, plot=True)
