...
def process_batch(batch, model, metadata, device):
    """Common batch processing logic for train and test."""
    states = batch[:, :, :metadata['n_state_features']]
    controls = batch[:, :, -metadata['n_control_features']:]
    init_states = states[:, 0, :]
    ts = torch.linspace(0, 1.0, states.shape[1]).to(device)
    
    predictions = model.predict(init_states, controls[:, 0], ts)
...
def train(dataloader, model, opt, scheduler, criterion, metadata, device, min_lr=5e-5):
    """Train the NODE model for one epoch."""
    model.train()
    total_loss = 0.0
    ...
    for batch in dataloader:
        ...
        predictions, states = process_batch(batch, model, metadata, device)
...
def test(dataloader, model, criterion, metadata, device):
    """Evaluate the NODE model on validation/test data."""
    model.eval()
    total_loss = 0.0
...
def main():
    ...
    model = NODE(config['model'], metadata).to(device)
    ...
    for epoch in range(start_epoch, config['training']['n_epochs']):
        ...
        if epoch % config['training']['save_interval'] == 0:
            ...
            # Call the prediction function on each trajectory
