...
from scripts.pirom_dyn.dyn_train import DPJ, DPT, B, f, g, mdl_kl, t_grid, trn_nd
...
def prepare_workdir(root: Path):
    stage_workdir(root, BASE_DIR, ["dyn_data.yaml", "dyn_model.yaml", "dyn_test.yaml"])


def generate_data(root: Path, seed: int | None = None):
    sampler = TrajectorySampler(f, g, config=root / "dyn_data.yaml", rng=seed)
    ts, _, us, ys, ps = sampler.sample(t_grid, batch=B)
    np.savez_compressed(root / "data" / "dyn.npz", t=ts, x=ys, u=us, p=ps)
    print(f"Generated data: {root / 'data' / 'dyn.npz'}")
...
def train(selected: list[int], root: Path):
    for idx in selected:
        case = cases[idx]
        opt = {
            "data": {"path": str(root / "data" / "dyn.npz")},
...
def main():
    ...
    if not (root / "data" / "dyn.npz").exists():
