...
from dymad.utils import JaxWrapper, TrajectorySampler, plot_multi_trajs
...
class DPT(TemplateCorrDif):
    ...
    def encoder(self, w) -> torch.Tensor:
        return torch.cat(
            [w.x, torch.zeros(*w.x.shape[:-1], 1, device=w.x.device, dtype=w.x.dtype)], dim=-1
        )
...
def main():
    ...
    if ifviz:
        for i in IDX:
            ...
            visualize_model(
                mdl_class=MDL,
                checkpoint_path=f"dyn_{mdl}.pt",
                ref_data="data/dyn.npz",
                depth=1,
                ifsave=True,
            )
    ...
    if ifprd:
        ...
        plot_multi_trajs(
            np.array(res), t_data, "DP", us=u_data, labels=["Truth"] + labels, ifclose=False
        )
