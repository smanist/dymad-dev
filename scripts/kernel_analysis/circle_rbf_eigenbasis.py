"""Convergence of RBF-kernel eigensystems on the unit circle.

Run this file directly. It writes a plot under ``runs/`` beside the script,
or under ``DYMAD_KERNEL_ANALYSIS_OUTPUT`` when that environment variable is set.
"""

from __future__ import annotations

import os
from pathlib import Path

import matplotlib

matplotlib.use("Agg")
import matplotlib.pyplot as plt
import numpy as np
import torch
from scipy.special import iv

from dymad.kernel_analysis import KernelEigenbasis
from dymad.modules import KernelScRBF

ifrun = True
ifplt = True
SAMPLE_SIZES = (16, 32, 64)
SIGMA = 0.35
N_COMPONENTS = 5
SOLVERS = ("dense", "matrix_free", "scipy")
OUTPUT_ROOT = Path(os.environ.get("DYMAD_KERNEL_ANALYSIS_OUTPUT", Path(__file__).with_name("runs")))


def circle_points(theta: np.ndarray) -> torch.Tensor:
    return torch.tensor(np.column_stack((np.cos(theta), np.sin(theta))), dtype=torch.float64)


def projector_error(estimated: torch.Tensor, exact: torch.Tensor) -> float:
    q_est, _ = torch.linalg.qr(estimated)
    q_exact, _ = torch.linalg.qr(exact)
    return float(torch.linalg.matrix_norm(q_est @ q_est.T - q_exact @ q_exact.T))


def align_degenerate_modes(estimate: torch.Tensor, truth: torch.Tensor) -> torch.Tensor:
    aligned = estimate.clone()
    for indices in ((0,), (1, 2), (3, 4)):
        selected = list(indices)
        numerical = estimate[:, selected]
        reference = truth[:, selected]
        if len(selected) == 1:
            aligned[:, selected] = numerical if (numerical * reference).sum() >= 0 else -numerical
        else:
            left, _, right_t = torch.linalg.svd(numerical.T @ reference)
            aligned[:, selected] = numerical @ (left @ right_t)
    return aligned


def solve_all(points: torch.Tensor) -> dict[str, KernelEigenbasis]:
    weights = torch.full((points.shape[0],), 1.0 / points.shape[0], dtype=torch.float64)
    return {
        solver: KernelEigenbasis(
            KernelScRBF(in_dim=2, lengthscale_init=SIGMA, dtype=torch.float64), N_COMPONENTS
        ).solve(points, sample_weights=weights, solver=solver, seed=0)
        for solver in SOLVERS
    }


if __name__ == "__main__" and ifrun:
    records: list[tuple[int, float, float]] = []
    exact_values = np.exp(-1.0 / SIGMA**2) * np.asarray(
        [
            iv(0, 1.0 / SIGMA**2),
            iv(1, 1.0 / SIGMA**2),
            iv(1, 1.0 / SIGMA**2),
            iv(2, 1.0 / SIGMA**2),
            iv(2, 1.0 / SIGMA**2),
        ]
    )
    for count in SAMPLE_SIZES:
        theta = 2.0 * np.pi * np.arange(count) / count
        points = circle_points(theta)
        truth = torch.tensor(
            np.column_stack(
                (
                    np.ones_like(theta),
                    np.sqrt(2.0) * np.cos(theta),
                    np.sqrt(2.0) * np.sin(theta),
                    np.sqrt(2.0) * np.cos(2.0 * theta),
                    np.sqrt(2.0) * np.sin(2.0 * theta),
                )
            ),
            dtype=torch.float64,
        ) / np.sqrt(count)
        solutions = solve_all(points)
        basis = solutions["dense"]
        value_error = max(
            abs(float(basis.eigenvalues[index]) - exact_values[index])
            for index in range(N_COMPONENTS)
        )
        records.append(
            (count, value_error, projector_error(basis.eigenvectors[:, 1:3], truth[:, 1:3]))
        )
        last_theta, last_truth, last_solutions = theta, truth, solutions

if __name__ == "__main__" and ifplt:
    OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)
    sizes, value_errors, subspace_errors = map(np.asarray, zip(*records, strict=True))
    figure, axes = plt.subplots(1, 2, figsize=(8, 3.2), constrained_layout=True)
    axes[0].loglog(sizes, np.maximum(value_errors, 1.0e-16), marker="o")
    axes[0].set(title="Eigenvalue error", xlabel="reference points", ylabel="absolute error")
    axes[1].loglog(sizes, np.maximum(subspace_errors, 1.0e-16), marker="o")
    axes[1].set(title="m=1 eigenspace error", xlabel="reference points", ylabel="projector norm")
    figure.savefig(OUTPUT_ROOT / "circle_rbf_eigenbasis_convergence.png", dpi=160)
    plt.close(figure)

    figure, axes = plt.subplots(1, 2, figsize=(8, 3.2), constrained_layout=True)
    mode = np.arange(N_COMPONENTS)
    axes[0].plot(mode, exact_values, color="black", marker="x", label="truth")
    for solver, basis in last_solutions.items():
        axes[0].plot(mode, basis.eigenvalues.cpu(), marker="o", linestyle="--", label=solver)
    axes[0].set(title="First eigenvalues", xlabel="mode", ylabel="eigenvalue")
    axes[0].legend(fontsize="small")
    for solver, basis in last_solutions.items():
        axes[1].semilogy(
            mode,
            np.maximum(np.abs(basis.eigenvalues.cpu().numpy() - exact_values), 1.0e-16),
            marker="o",
            label=solver,
        )
    axes[1].set(title="Eigenvalue error", xlabel="mode", ylabel="absolute error")
    axes[1].legend(fontsize="small")
    figure.savefig(OUTPUT_ROOT / "circle_rbf_eigenbasis_eigenvalues.png", dpi=160)
    plt.close(figure)

    figure, axes = plt.subplots(
        N_COMPONENTS, 1, figsize=(8, 8), sharex=True, constrained_layout=True
    )
    for index, axis in enumerate(axes):
        axis.plot(last_theta, last_truth[:, index], color="black", linewidth=2.0, label="truth")
        for solver, basis in last_solutions.items():
            aligned = align_degenerate_modes(basis.eigenvectors, last_truth)
            axis.plot(last_theta, aligned[:, index], linestyle="--", label=solver)
        axis.set_ylabel(f"mode {index}")
    axes[0].legend(ncol=4, fontsize="small")
    axes[-1].set_xlabel("angle")
    figure.savefig(OUTPUT_ROOT / "circle_rbf_eigenbasis_eigenvectors.png", dpi=160)
    plt.close(figure)
