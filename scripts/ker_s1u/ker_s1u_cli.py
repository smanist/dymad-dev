...
def parse_args():
    parser = argparse.ArgumentParser(description="Run kernel S1U cases.")
...
def predict(selected):
    ...
    for i in selected:
        ...
        res.append(pred)
    plot_trajectory(np.array(res), t_data, "S1U", us=u_data, labels=["Truth"] + [cfgs[i][0] for i in selected], ifclose=False)
