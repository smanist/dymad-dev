...
class KRRBase(nn.Module):
    ...
    def _restore_loaded_inference_state(self) -> None:
        ...
        # Keep the restored training inputs as ordinary runtime state. Assigning
        # the ``Parameter`` itself would register ``X_train`` as a new parameter,
        # so a second strict state-dict load would incorrectly require an
        # ``X_train`` key that was absent from the original checkpoint.
        self.X_train = self._Xref.detach()
        self.Y_train = None
        self._residual = None
...
class KRRMultiOutputIndep(KRRBase):
    ...
    def _ensure_solved(self):
        ...
        while unsolved:
            ...
            first_system = (
                cast(KernelAbstract, kernels[first])(X, None)
            ...
            for d in candidates:
                candidate_system = (
                    cast(KernelAbstract, kernels[d])(X, None) + (self.ridge[d] + self.jitter) * I
                )
