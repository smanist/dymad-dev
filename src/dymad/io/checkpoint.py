...
class DataInterface:
    ...
    def encode(self, X: np.ndarray, rng: list | None = None) -> np.ndarray:
        ...
        """
        return self._transform_state_array(X, rng=rng)

    def decode(self, X: np.ndarray, rng: list | None = None) -> np.ndarray:
        """
        Decode trajectory data from the observer space.
        """
        return self._transform_state_array(X, rng=rng, inverse=True)

    def _transform_state_array(
        self,
        X: np.ndarray,
        *,
        rng: list | None = None,
        ...
        array = np.asarray(X)
        if array.ndim <= 2:
            payloads = [np.atleast_2d(array)]
        else:
            payloads = [np.atleast_2d(item) for item in array]
        transform = self._trans_x.inverse_transform if inverse else self._trans_x.transform
        outputs = transform(payloads, rng)
        if len(outputs) == 1:
