...
def _ls_sako(A: np.ndarray, b: np.ndarray, params=None) -> tuple[np.ndarray, np.ndarray]:
    ...
    logger.info(
        f"SAKO filtered {len(_w) - len(eigs)} out of {len(_w)} eigenvalues. Max residual: {max(res[0]):3.1e}"
    )
...
class LSUpdater:
    ...
    def __init__(self, method, model, dt=None, params=None, **kwargs):
        ...
        if not check_linear_impl(model):
            raise ValueError(
                f"{model} does not implement linear_features and linear_eval methods required for LS updates."
            )
        ...
        if check_linear_solve(model):
            ...
            if self.method not in SOL_MAP:
                raise ValueError(
                    f"Unsupported method: {self.method}. Supported methods are {list(SOL_MAP.keys())}."
                )
            self.solver = SOL_MAP[self.method]
