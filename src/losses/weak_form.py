...
def weak_form_loss(batch, predBatch, metadata, criterion):
    ...
    1. Processes each trajectory in the batch separately but efficiently
    2. For each trajectory, applies sliding windows of size N with stride dN
    3. Enforces C*w ≈ D*w_dot for each window (weak form consistency)
    4. Also enforces reconstruction accuracy via decoder loss
    5. Optimized to reuse C and D matrices and minimize tensor operations
    """
